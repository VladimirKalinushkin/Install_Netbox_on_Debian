...
For more information see https://netboxlabs.com/docs/netbox/.

For wach actual release see https://github.com/netbox-community/netbox/releases
...
If you want to install Netbox by auto, use the script `Install_netbox.sh`
...
But if you want install all manually, use this insthuctions:
...
`redis-cli` to the Redis server using the following AUTH query and be sure to change the password. `AUTH YourPasswordForRedis` Once authenticated, you should get the output OK Now run the PING query below to ensure that your connection is successful `PING` If successful, you should get the output PONG from the Redis server. `EXIT`
...
#### 5.c.1. You should copy fix_debug_with_authorisation_users.py from instalation media to /opt/netbox/netbox/

```
cp Includes/fix_debug_with_authorisation_users.py \
...
```
cp /opt/netbox/contrib/nginx.conf
